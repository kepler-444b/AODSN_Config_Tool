...
// 设置按钮状态
void ModelPanel6Key::SetPanelState(const QByteArray &data)
{
...
bool ModelPanel6Key::eventFilter(QObject *obj, QEvent *event)
{
    ...
    else if (selected == deleteAction) // 删除面板
    {
...
void ModelPanel6Key::on_key_1_clicked()
{
    ui->key_1->setChecked(!ui->key_1->isChecked()); // 状态回滚
    bulid_panel_frame(0);
}

void ModelPanel6Key::on_key_2_clicked()
{
    ui->key_2->setChecked(!ui->key_2->isChecked()); // 状态回滚
    bulid_panel_frame(1);
}

void ModelPanel6Key::on_key_3_clicked()
{
    ui->key_3->setChecked(!ui->key_3->isChecked()); // 状态回滚
    bulid_panel_frame(2);
}

void ModelPanel6Key::on_key_4_clicked()
{
    ui->key_4->setChecked(!ui->key_4->isChecked()); // 状态回滚
    bulid_panel_frame(3);
}

void ModelPanel6Key::on_key_5_clicked()
{
    ui->key_5->setChecked(!ui->key_5->isChecked()); // 状态回滚
    bulid_panel_frame(4);
}

void ModelPanel6Key::on_key_6_clicked()
{
    ui->key_6->setChecked(!ui->key_6->isChecked()); // 状态回滚
    bulid_panel_frame(5);
}

// 构造面板通讯帧
void ModelPanel6Key::bulid_panel_frame(const uint8_t key_num)
{
